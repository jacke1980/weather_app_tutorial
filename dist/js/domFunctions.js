...
const createCurrentConditionsDivs = (weatherObj, unit) =>{
    ...
    const icon = createMainImgDiv(weatherObj.current.weather[0].icon,weatherObj.
    ...
    const feels = createElem("div", "feels", `Feels like ${Math.round(Number(weatherObj.
        current.feels_like))}•`);
    const maxTemp = createElem("div", "maxtemp",`High ${Math.round(Number(weatherObj.daily[0].
        temp.max))}•`);
    const minTemp = createElem("div", "mintemp", `Low ${Math.round(Number(weatherObj.daily[0].
        temp.min))}•`);
    const humidity = createElem("div","humidity",`Humidity ${weatherObj.current.humidity}%`);
    const wind = createElem("div","wind", `wind ${Math.round(Number(weatherObj.current.
        wind_speed))} ${windUnit}`);
...
const createDailyForecastDivs = (dayWeather)=>{
    ...
    const dayIcon = createDailyForecastIcon(dayWeather.weather[0].icon,dayWeather.weather[0].
        description);
